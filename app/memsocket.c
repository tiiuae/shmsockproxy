...
int get_shmem_size(int instance_no) {
  ...
  if (lseek(shmem_fd[instance_no], 0, SEEK_SET) < 0) {
    FATAL("seek");
  }
  return res;
}
...
extern ssize_t vmsplice(int fd, const struct iovec *iov,
            ...
            for (tmp = 0; tmp < peer_shm->len; tmp += PAGE_SIZE) {
              ...
              rv = lseek(shmem_fd[instance_no], 0 /*offset*/, SEEK_SET);
              if (rv < 0) {
                ERROR("lseek %d errno=%d", rv, errno);
              }

              rv = sendfile(conn_fd, shmem_fd[instance_no], /*NULL*/ &offset, data_chunk);
